...
> 在一段基础搜索展开流中，是否有某个真实 `h` 桶长期占据较高比例，同时搜索很少逃逸到更低的 `h` 区域？
...
| `PLATEAU_MIN_SHARE` | 0.3 | 某桶在窗口中的最小占比 |
| `PLATEAU_MAX_LOWER_SHARE` | 0.1 | 低于该桶的展开所允许的最大占比 |
...
某个桶需要在 3 个连续窗口中同时满足最少样本数、最小占比和最大 lower-share，才进入 `active + armed`。检测器在记录当前展开到窗口之前先检查已有 active 桶，因此“刚好完成第三个确认窗口的状态”不会被立即发送；之后第一个落入该桶且满足全局冷却和预算的状态，才成为 LLM 请求源。

### 4.4 这个设计保留的核心哲学

- 检测的仍然是“搜索长时间被困在某个启发式高度”，而不是简单的定时请求。
- 展开数窗口比 wall-clock 窗口更少受其他系统负载影响，但不同问题的单次展开成本不同，所以它不等价于固定时间间隔。
- 通过 lower-share 限制，“高占比”还需同时表现为很少进入更低 `h`，从而区分“搜索访问很多同高度状态”与“搜索实际在下降”。
...
- `scale <= 30`：默认时限 1800 秒、每 phase 10 次 LLM 请求，默认最多 2 个搜索器并行共享 vLLM；
- `scale > 30`：默认时限 3600 秒、每 phase 15 次 LLM 请求，作为 exclusive barrier，等待先前所有问题结束后单独运行；
...
- 当前 65536/3/0.3/500000 等参数在 Depots 以外也有效。
- 30 规模并行 2 个搜索器、40 规模请求预算 15 是通用最优配置。
