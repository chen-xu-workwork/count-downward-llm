...
void PartialStateTreeNode::add(
    const vector<Fact> &partial_state,
    ...
    */
    ...
    for (const Fact &fact : partial_state) {
        if (fact.var == var_id) {
...
bool PartialStateTreeNode::contains(const vector<Fact> &partial_state) const {
    assert(is_sorted(partial_state.begin(), partial_state.end()));
    if (var_id == DEAD_END_LEAF) {
        return true;
    }
...
void PartialStateTree::add(
    const vector<Fact> &partial_state, const vector<int> &domain_sizes) {
    vector<int> uncovered_vars;
...
bool PartialStateTree::subsumes(const vector<Fact> &partial_state) const {
