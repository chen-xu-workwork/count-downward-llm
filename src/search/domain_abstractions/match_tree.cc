...
MatchTree::MatchTree(const vector<int> &domain_sizes,
...
void MatchTree::insert_recursive(
    int op_id, const vector<Fact> &regression_preconditions,
    ...
    if (pre_index == static_cast<int>(regression_preconditions.size())) {
        ...
        const Fact &fact = regression_preconditions[pre_index];
        int var_id = fact.var;
        int var_domain_size = domain_sizes[var_id];
        if (var_domain_size == 1) {
            /*
              The variable of this fact is trivial (i.e., has only one
              value). Hence, it makes no sense to include it in the
              match tree and we simply ignore it.
            */
            insert_recursive(op_id, regression_preconditions,
                             ++pre_index, edge_from_parent);
            return;
        }

        // Set up node correctly or insert a new node if necessary.
        if (node->is_leaf_node()) {
            node->initialize(var_id, var_domain_size);
        } else if (node->var_id > var_id) {
            /* The variable to test has been left out: must insert new
               node and treat it as the "node". */
            Node *new_node = new Node();
            new_node->initialize(var_id, var_domain_size);
...
void MatchTree::insert(int op_id, const vector<Fact> &regression_preconditions) {
