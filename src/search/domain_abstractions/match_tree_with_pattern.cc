...
void MatchTreeWithPattern::insert(int op_id, const vector<Fact> &regression_preconditions) {
...
void MatchTreeWithPattern::dump_recursive(Node *node) const {
    if (!node) {
        // Node is the root node.
        cout << "Empty MatchTreeWithPattern" << endl;
        return;
    }
    cout << endl;
...
void MatchTreeWithPattern::dump() const {
    dump_recursive(root);
}
}
