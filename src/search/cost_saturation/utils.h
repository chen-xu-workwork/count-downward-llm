...
extern bool is_sum_within_range(ap_float a, ap_float b);

// The sum of mixed infinities evaluates to the left infinite value.
extern ap_float left_addition(ap_float a, ap_float b);

extern ap_float compute_max_h(
...
extern void reduce_costs(
    std::vector<ap_float> &remaining_costs, const std::vector<ap_float> &saturated_costs);

extern std::vector<ap_float> get_operator_costs(const TaskProxy &task_proxy);
...
extern std::shared_ptr<ScalarEvaluator> get_max_cp_heuristic(
