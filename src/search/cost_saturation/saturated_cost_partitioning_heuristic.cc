...
CostPartitioningHeuristic compute_saturated_cost_partitioning(
    ...
    vector<ap_float> &remaining_costs,
    const vector<int> &) {
    assert(abstractions.size() == order.size());
    CostPartitioningHeuristic cp_heuristic;
    for (int pos : order) {
        const Abstraction &abstraction = *abstractions[pos];
        vector<ap_float> h_values = abstraction.compute_goal_distances(remaining_costs);
...
static void cap_h_values(ap_float h_cap, vector<ap_float> &h_values) {
    assert(h_cap != -INF);
    for (ap_float &h : h_values) {
        if (h != INF) {
            h = min(h, h_cap);
        }
    }
...
CostPartitioningHeuristic compute_perim_saturated_cost_partitioning(
    ...
    for (int pos : order) {
        const Abstraction &abstraction = *abstractions[pos];
        vector<ap_float> h_values = abstraction.compute_goal_distances(remaining_costs);
        ap_float h_cap = h_values[abstract_state_ids[pos]];
...
CPFunction get_cp_function_from_options(const Options &opts) {
    Saturator saturator_type = static_cast<Saturator>(opts.get<int>("saturator"));
...
static Heuristic *_parse(OptionParser &parser) {
    parser.document_synopsis(
        ...
        utils::format_paper_reference(
            ...
            "Journal of Artificial Intelligence Research 67",
            "129-167",
            "2020"));
...
static Plugin<Heuristic> _plugin("scp", _parse);
}
