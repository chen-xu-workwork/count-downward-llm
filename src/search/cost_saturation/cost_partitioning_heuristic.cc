...
void CostPartitioningHeuristic::add_h_values(
    int abstraction_id, vector<ap_float> &&h_values) {
    if (any_of(h_values.begin(), h_values.end(), [](ap_float h) {return h > 0;})) {
...
void CostPartitioningHeuristic::merge_h_values(
    int abstraction_id, vector<ap_float> &&h_values) {
    if (any_of(h_values.begin(), h_values.end(), [](ap_float h) {return h > 0 && !isinf(h);})) {
        ...
        if (lookup_table_id == -1) {
            ...
            vector<ap_float> &old_h_values = lookup_tables[lookup_table_id].h_values;
            assert(h_values.size() == old_h_values.size());
            for (size_t i = 0; i < h_values.size(); ++i) {
                ap_float &h1 = old_h_values[i];
                ap_float h2 = h_values[i];
                h1 += h2;
            }
        }
    }
...
ap_float CostPartitioningHeuristic::compute_heuristic(
...
int CostPartitioningHeuristic::estimate_size_in_kb() const {
    return (get_num_heuristic_values() * sizeof(ap_float) +
            lookup_tables.size() * sizeof(vector<ap_float>)) / 1024.;
}
