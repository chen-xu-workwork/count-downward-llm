...
static vector<Fact> get_projected_conditions(
    const ConditionsProxy &conditions, const pdbs::Pattern &pattern) {
    vector<Fact> relevant_conditions;
...
struct ProjectedEffect {
    Fact fact;
    vector<Fact> conditions;
    bool always_triggers;

    ProjectedEffect(
        const Fact &projected_fact,
        vector<Fact> &&conditions,
        bool always_triggers)
        : fact(projected_fact),
...
static vector<vector<Fact>> get_relevant_preconditions_by_operator(
    const OperatorsProxy &ops, const pdbs::Pattern &pattern) {
    vector<vector<Fact>> preconditions_by_operator;
...
bool ExplicitProjectionFactory::conditions_are_satisfied(
    const vector<Fact> &conditions, const UnrankedState &state_values) const {
    return all_of(conditions.begin(), conditions.end(), [&state_values](const Fact &precondition) {
...
void ExplicitProjectionFactory::add_transitions(
    ...
    utils::HashSet<Fact> possible_effects;
    ...
    for (int mask = 0; mask < powerset_size; ++mask) {
        ...
        int i = 0;
        for (const Fact &fact : possible_effects) {
            if (mask & (1 << i)) {
