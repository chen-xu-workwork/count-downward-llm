...
extern ap_float compute_stolen_costs(ap_float wanted_by_abs, ap_float surplus_cost);
...
extern double compute_score(
    ap_float h, ap_float used_costs, ScoringFunction scoring_function);

/*
  surplus(o) = sum of costs that each heuristic wants from o - cost(o).
*/
extern std::vector<ap_float> compute_all_surplus_costs(
    const std::vector<ap_float> &costs,
    const std::vector<std::vector<ap_float>> &saturated_costs_by_abstraction);

/*
  Compute total amount of costs that a heuristic (with the given saturated cost
  function) "steals" from the other heuristics.
*/
extern ap_float compute_costs_stolen_by_heuristic(
    const std::vector<ap_float> &saturated_costs,
    const std::vector<ap_float> &surplus_costs);
