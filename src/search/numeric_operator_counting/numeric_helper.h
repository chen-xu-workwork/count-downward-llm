...
class NumericTaskProxy {
 public:
  ...
  const std::list<std::pair<int, int>> &get_propositional_goals(int id_goal) const { return propositional_goals[id_goal]; }
  const std::list<int> &get_numeric_goals(int id_goal) const { return numeric_goals[id_goal]; }
  size_t get_n_numeric_conditions() const { return numeric_conditions.size(); }
  const std::list<int> &get_numeric_conditions_id(int pre_id) const {
  ...
  const std::set<int> &get_achievers(int fact_id) const { return achievers[fact_id]; }
  const std::string &get_proposition_name(int p_id) const { return proposition_names[p_id]; }
  double get_small_m(int p_id) const { return small_m[p_id]; }
  double get_epsilon(int p_id) const { return epsilon[p_id]; }
  const std::set<int> &get_mutex_actions(int op_id) const { return mutex_actions[op_id]; }
