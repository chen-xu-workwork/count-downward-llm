...
namespace numeric_pdbs
{
    ...
    void PatternCollectionGeneratorGenetic::transform_to_pattern_bitvector_form(vector<bool> &bitvector, const Pattern &pattern, numeric_pdb_helper::NumericTaskProxy &task_proxy) const
    {
    ...
    void PatternCollectionGeneratorGenetic::mutate2(numeric_pdb_helper::NumericTaskProxy &task_proxy)
    {
        ...
        for (size_t i = 0; i < pattern_collections.size(); ++i)
        {
            for (size_t j = 0; j < pattern_collections[i].size(); ++j)
            {
                ...
                if (is_pattern_too_large(trans_pattern, task_proxy))
                {
                    ...
                    for (size_t i = 0; i < trans_pattern.regular.size(); ++i)
                    {
                        pattern_vars.push_back(trans_pattern.regular[i]);
                    }
                    for (size_t i = 0; i < trans_pattern.numeric.size(); ++i)
                    {
                        pattern_vars.push_back(trans_pattern.numeric[i] + num_vars);
                    }
