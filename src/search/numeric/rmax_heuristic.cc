...
ap_float RMaxHeuristic::compute_heuristic(const State &state) {
...
RMaxHeuristic::RMaxHeuristic(const std::shared_ptr<AbstractTask> &task)
        : Heuristic(task) {
    restrict_achievers = false;
    ceiling_less_than_one = false;
