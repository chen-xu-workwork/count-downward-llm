...
#    ("landmarks", ["--search", "astar(operatorcounting([lm_numeric],cplex,lp))"]),
    ("lmcut-ipc-oss", ["--search", f"orbit_astar(lmcutnumeric(use_second_order_simple=true, bound_iterations=10, ceiling_less_than_one=true), symmetry=symmetry_state_pruning(goal_only))"]),
...
for coll_name, coll_limit in [#("100k", 100000), ("1M", 1000000), 
                              ("5M", 5000000), #("10M", 10000000)
                              ]:
    for pdb_name, pdb_limit in [#("1k", 1000), ("10k", 10000), ("50k", 50000), 
                                ("100k", 100000), #("200k", 200000)
                                ]:
        if coll_limit > pdb_limit:
            CONFIGS.append((f"ipdb-{coll_name}-{pdb_name}-oss", ["--search", f"orbit_astar(numeric_ipdb(max_time=900, max_number_pdb_states={pdb_limit}, collection_max_size={coll_limit}), symmetry=symmetry_state_pruning(goal_only))"]))
...
exp.add_suite(BENCHMARKS_DIR_OTHERS, ["petri-net"])
