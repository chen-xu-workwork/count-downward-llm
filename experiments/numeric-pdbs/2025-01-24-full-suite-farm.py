...
BENCHMARKS_DIR = "/proj/parground/users/x_mfrit/benchmarks/baseline"
...
prefix = "astar(numeric_ipdb(max_time=900, extension_h0_until_goal=100, extension_h1_until_goal=100, "

postfix = "))"

f_layer_offsets = ["f_layer_offset_ratio=-1.0", "f_layer_offset_ratio=0.1"]
extend_abstract_state_space = ["extend_abstract_state_space=1", "extend_abstract_state_space=0"]
drop_pdb = ["drop_pdb=1", "drop_pdb=0"]
blind_if_no_goal = ["blind_if_no_goal=1", "blind_if_no_goal=0"]
...
lmcut_configs = [
    "astar(numeric_ipdb(max_time=900, use_lmcut=1, f_layer_offset_ratio=-1.0, extend_abstract_state_space=1, blind_if_no_goal=1))",
    "astar(numeric_ipdb(max_time=900, use_lmcut=1, f_layer_offset_ratio=-1.0, extend_abstract_state_space=0, blind_if_no_goal=1))",
    "astar(numeric_ipdb(max_time=900, use_lmcut=1, f_layer_offset_ratio=-1.0, extend_abstract_state_space=1, blind_if_no_goal=0))",
    "astar(numeric_ipdb(max_time=900, use_lmcut=1, f_layer_offset_ratio=-1.0, extend_abstract_state_space=0, blind_if_no_goal=0))",
]

configs.extend(lmcut_configs)

configs = [(f"iPDB-{num}", ["--search", config]) for num, config in enumerate(configs)]
...
for rev in REVISIONS:
    for config_nick, config in configs:
        add_trans = ["--restricted-task-transformation"]
        exp.add_algorithm(
            f"{config_nick}-{rev[:5]}",
            REPO,
            rev,
...
exp.add_suite(BENCHMARKS_DIR, project.SUITE_FARMLAND_OTHERS)
