...
REVISIONS = ["bdb66f9756364f76bf1d734bbf3cf98151c63b5d"
            ]
...
for rev in REVISIONS:
    for config_nick, config in CONFIGS:
        for extra_driver_opt in [[], ["--restricted-task-transformation"]]:
            exp.add_algorithm(
                f"{config_nick}-{rev[:5]}{'T' if extra_driver_opt else ''}",
                REPO,
                rev,
                config,
                build_options=BUILD_OPTIONS,
                driver_options=DRIVER_OPTIONS + extra_driver_opt,
            )
