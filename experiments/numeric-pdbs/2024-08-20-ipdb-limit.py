...
REVISIONS = ["ae712742f13145677dc37b31319b6799aedadaa5"
            ]
...
for coll_name, coll_limit in [#("100k", 100000), ("1M", 1000000), 
    ...
    for pdb_mns_name, pdb_max_num_states in [#("10k", 10000), 
                                ("100k", 100000), #("1M", 1000000)
                                ]:
        for pdb_ms_name, pdb_max_size in [("1M", 1000000), ("2M", 2000000), ("10M", 10000000)]:
            for max_time in [900]:
                if coll_limit > pdb_max_num_states and pdb_max_num_states <= pdb_max_size:
                    CONFIGS.append((f"ipdb-{coll_name}-{pdb_mns_name}-{pdb_ms_name}-{max_time}", ["--search", f"astar(numeric_ipdb(max_time={max_time}, max_pdb_size={pdb_max_size}, max_number_pdb_states={pdb_max_num_states}, collection_max_size={coll_limit}))"]))
                    CONFIGS.append((f"ipdb-{coll_name}-{pdb_mns_name}-{pdb_ms_name}-{max_time}-nodom", ["--search", f"astar(numeric_ipdb(dominance_pruning=false, max_time={max_time}, max_pdb_size={pdb_max_size}, max_number_pdb_states={pdb_max_num_states}, collection_max_size={coll_limit}))"]))
