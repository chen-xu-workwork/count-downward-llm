...
exp.add_fetcher("data/2024-08-14-petrinet-eval", name="fetch-petrinet", merge=True)
exp.add_fetcher("data/2024-08-15-aaai-ipdb-20m-eval", name="fetch-ipdb20m", merge=True)


def filter_final_domains(run):
    domain = run["domain"]
    if domain in ["markettrader_test_10", "pn_domain"]:
        return False
